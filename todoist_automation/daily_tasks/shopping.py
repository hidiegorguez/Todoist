...
def process_compra_subtasks(tf, all_tasks):
    ...
    compra_task = next((task for task in all_tasks if task.id == config.COMPRA_TASK_ID), None)
    ...
    compra_subtasks = list(filter(lambda task: task.parent_id == config.COMPRA_TASK_ID, all_tasks))
    for subtask in compra_subtasks:
        missing_labels = [label for label in config.COMPRA_REQUIRED_LABELS if label not in subtask.labels]
        if missing_labels:
        ...
        if subtask_due_date is not None and subtask_due_date < compra_due_date:
            ...
            try:
                if original_is_recurring and original_due_string:
                    ...
                    tf.update_task(task_id=subtask.id, due_date=compra_due_str)
